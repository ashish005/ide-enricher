/**
 * Created by wizdev on 3/25/2016.
 */
(function(define, angular){
    "use strict";
    var _rootPath = 'client/components/';
    function ideTreeFactory() {
        var obj = this;
        obj.model = (function() {
                ...
                exportZip : function(entry, onend, onprogress, onerror) {
                    ...
                    if (requestFileSystem)
                        createTempFile(function(fileEntry) {
                            ...
                            entry.fs.exportFileEntry(zipFileEntry, onexport, onprogress, onerror);
                        });
                    else
                        entry.fs.exportBlob(onexport, onprogress, onerror);
                },
    ...
    function ideTree(ideTreeFactory) {
        return {
            restrict: 'AE',
            scope:{
                sendNodeInfo:'&'
            },
            templateUrl: _rootPath+'tree/templates/ideTree.html',
            link: function ($scope, element, attr, ctrl, transclude) {
                var model = ideTreeFactory['model'];
                (function() {
                    ...
                    function refreshTree(node, element) {
                        var details, summary, label, newDirectory, exportDirectory, icon;
    ...
    var _codeTree = 'goLive.tree';
    angular
        .module(_codeTree, [])
        .factory('ideTreeFactory',ideTreeFactory)
        .directive('ideTree', ['ideTreeFactory', ideTree])
